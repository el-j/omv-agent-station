...
class TestSignalBindUnbind(unittest.TestCase):
    ...
    def test_pull_falls_back_to_bound_project(self):
        async def scenario():
            ...
            orig = self.signal_bot.git_pull_repo
            self.signal_bot.git_pull_repo = fake_git_pull_repo
            try:
                await self.signal_bot.handle_signal_command(sender, "/pull")
                self.assertTrue(any("myproj" in m for _, m in self.sent))
            finally:
                self.signal_bot.git_pull_repo = orig
...
class TestSignalFileUpload(unittest.TestCase):
    ...
    def test_rejects_when_no_project_bound(self):
        async def scenario():
            sender = "+15550002001"
            await self.signal_bot.handle_signal_upload(sender, {"id": "att-1", "filename": "x.txt", "size": 10}, None)
    ...
    def test_rejects_oversized_attachment(self):
        async def scenario():
            ...
            big_size = self.signal_bot.MAX_UPLOAD_BYTES + 1
            await self.signal_bot.handle_signal_upload(sender, {"id": "att-1", "filename": "x.txt", "size": big_size}, None)
    ...
    def test_rejects_path_traversal_caption(self):
        async def scenario():
            sender = "+15550002003"
            self.topics_service.set_bound_project(sender, None, "myproj")
            await self.signal_bot.handle_signal_upload(
    ...
    def test_full_upload_flow_downloads_commits_and_pushes(self):
        async def scenario():
            ...
            orig_download = self.signal_bot.download_signal_attachment
            self.signal_bot.download_signal_attachment = fake_download
            ...
            try:
                await asyncio.wait_for(
                    self.signal_bot.handle_signal_upload(
                ...
                self.signal_bot.download_signal_attachment = orig_download
